...
class Spaz(ba.Actor):
    ...
    def _stop_run(self):
        try:
            self.node.run = 0
            self.startRunTimer = ba.Timer(0.05 / 3.0, self._start_run)
        except:
            self.stopRunTimer = None
            self.startRunTimer = None

    def _start_run(self):
        try:
            self.node.run = self.speed
        except:
            self.stopRunTimer = None
            self.startRunTimer = None
    ...
    def _protection(self):
        ...
        if self.protection:
            ...
            for player in activity.players:
                if player.actor and player.actor.node and player.actor.node.hold_node == self.node:
                    player.actor.node.hold_node = None
                    player.actor.node.hold_body = 0
            ...
            self.protection_light = Hologram(position=self.node.torso_position,
                                              model='locatorCircleOutline',
                                              texture='bonesColorMask',
                                              size=0.75,
                                              color=[3.0, 0.6, 0.0])
            mnode.connectattr('output', self.protection_light.node, 'position')
    ...
    def on_run(self, value: float) -> None:
        ...
        if value == 0:
            self.stopRunTimer = None
            self.startRunTimer = None
        elif self.speed > 1.1 and self.speed < 10.0:
            self.stopRunTimer = ba.Timer(0.45 / 3.0, self._stop_run, repeat=True)
        self.node.run = value * self.speed
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, ba.PickedUpMessage):
            if self.protection:
                def _safe_drop(node: ba.Node | None) -> None:
                    if node:
                        node.hold_node = None
                        node.hold_body = 0
                ba.timer(0.01, ba.Call(_safe_drop, msg.node))
                return None
            if self.node:
            ...
            if self.protection: return None
            ...
            if self.protection:
                try:
                    activity = ba.getactivity()
                    for i in range(len(activity.map.spawn_points)):
                        pos = self.node.torso_position
                        new_pos = (activity.map.get_start_position(i))
                        dist = (pos[0] - new_pos[0]) * (pos[0] - new_pos[0]) + (pos[1] - new_pos[1]) * (pos[1] - new_pos[1]) + (pos[2] - new_pos[2]) * (pos[2] - new_pos[2])
                        if i == 0 or dist < optimal_dist:
                            optimal_pos = new_pos
                            optimal_dist = dist
                    for i in range(1):
                        pos = self.node.torso_position
                        new_pos = activity.map.get_ffa_start_position([])
                        dist = (pos[0] - new_pos[0]) * (pos[0] - new_pos[0]) + (pos[1] - new_pos[1]) * (pos[1] - new_pos[1]) + (pos[2] - new_pos[2]) * (pos[2] - new_pos[2])
                        if dist < optimal_dist:
                            optimal_pos = new_pos
                            optimal_dist = dist
                    self.handlemessage(ba.StandMessage(optimal_pos, random.uniform(0, 360)))
                    return None
                except:
                    pass
