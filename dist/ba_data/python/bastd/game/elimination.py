            ...
            for team in self.teams:
                if len(team.players) == 1:
                    ...
                    for icon in player.icons:
                        icon.set_position_and_scale((xval, 65), 0.7)
                    ...
                    while True:
                        ...
                        for player in players_with_lives:
                            player.icons.append(
                                Icon(
                                    player,
                                    position=(xval, (75 if is_first else 60)),
        ...
        if isinstance(msg, ba.PlayerDiedMessage):
            ...
            if killer is not None:
                try:
                    ...
                    account_id = client_to_account(player.sessionplayer.inputdevice.client_id)
                    from ModData import stats
                    stats.increase_value(session.localPlayersData['stats'], account_id, 'deaths')
                    if killer and killer != player:
                        account_id = client_to_account(killer.sessionplayer.inputdevice.client_id)
                        stats.increase_value(session.localPlayersData['stats'], account_id, 'kills')
                except:
                    pass
