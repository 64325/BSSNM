...
description = 'изменить силу взрыва'
info = [
    '                             изменить силу взрыва',
    '                                    /bpower   число'
]
...
def run_command(client_id, account_id, type, command_keys, player):
    set_bomb_power(player, command_keys['value'])

def set_bomb_power(cl_id, value):
    activity = ba.getactivity()
    for player in activity.players:
        if player.sessionplayer.inputdevice.client_id == cl_id:
            if player.actor:
                player.actor.blast_radius = value